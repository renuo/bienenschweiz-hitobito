...
RSpec.describe Api::Mobile::V1::InspectionPdfController, type: :request do
  ...
  describe 'GET #show' do
    context 'existing qcontrol' do
      ...
      subject! { get api_mobile_v1_inspection_pdf_path(quality_control), params: { format: :json }, headers: auth_headers }
    ...
    context 'non-existing qcontrol' do
      subject! { get api_mobile_v1_inspection_pdf_path('12345'), params: { format: :json }, headers: auth_headers }

      it { expect(response).to have_http_status :not_found }
    end

    context 'qcontrol of a beekeeper who is not inspectable by the fachperson_produkte' do
      subject! { get api_mobile_v1_inspection_pdf_path(quality_control, format: :json), headers: auth_headers }

      let(:other_beekeeper) { Fabricate(:beekeeper, group_id: groups(:aarberg)) }
      let!(:quality_control) { Fabricate(:due_soon_qcontrol, member_id: other_beekeeper.id, author_id: fachperson_produkte.id) }
